    ...
    def test(self, opt, dataset):
        ...
        for i in range(0, len(dataset)):
            try:
                ...
                imtv = Variable(imtv.data.cuda(), requires_grad=True)
                cct = Variable(cct.data.cuda(), requires_grad=True)
                # imtv.requires_grad = True
                # cct.requires_grad = True
                ...
                # Call FGSM Attack
                if opt.perturb_audio:
                    perturbed_cct = fgsm_attack(cct, opt.epsilon, cct_grad)
                else:
                    perturbed_cct = cct
                ...
                f = "{}/{}/Grad.avi".format(opt.attack_dir, os.path.basename(videofile).strip('.avi'))
                grad_video = self.postprocess_grad(imtv.grad)
                self.savevid(grad_video, f, "")

                f = "{}/{}/Diff.avi".format(opt.attack_dir, os.path.basename(videofile).strip('.avi'))
